...
| [0028](0028-read-lexicon-retirement.md) | Retiring "audit" — the `read` lexicon + `catalog.` host | Accepted | Renames; re-decides nothing. Paid product becomes **The Catalog Letter** (Amendment 1 — "read" stays a verb only; word promoted from `concierge-deliverable.ts:56`, ADR 0022:16, and VOICE.md's doctor's-report register); `audit.flintmere.com` → `catalog.flintmere.com` (host = subject, routes = verbs); `/audit` → `/catalog-letter`. Ladder framing scan → read → fix per the workflow-moat reframe. "Audit" frozen in the six GDPR/SOC-2 legal lines, retained as an SEO target term, untouched in code identifiers. ADR 0022 prices unchanged. |

| [0029](0029-retail-gate-pivot.md) | The retail-gate pivot — from catalog readiness to supplier data onboarding | Accepted | Five of the April thesis's eight premises failed on re-test. Product becomes passage through the UK retailer product-data gate (Ocado's manual, verified unchanged May 2026: launch blocked until back-of-pack data is approved); the sale is knowledge of an unpublished schema plus carriage of the accuracy liability retailers assign to suppliers. Distribution partner-first — no warm network, <10 h/week. Retires the dead-inventory wedge, `suppressionEstimate`, "invisible to AI shopping", the £99–£499 ladder, `llms.txt` as a scored pillar, and the Shopify app as a near-term deliverable. Closes ADR 0019 as **Fail** via its own documented consulting-led path. Corrects two factual errors in ADR 0027. Band magnitudes re-opened, deferred to a later ADR. |

| [0030](0030-suppression-wedge-retirement.md) | Retiring the suppression wedge — surfaces, model, and revenue derivation | Accepted | Executes ADR 0029 §5's second clause, which named no surfaces. The wedge has two beats: a deterministic anchor ("N products are missing data Google Shopping looks for") and a probabilistic subline ("roughly X–Y are likely suppressed"). Only the second dies. `/scan` reframes to neutral catalog readiness — suppression framing leaves the lede, page title, OG/Twitter cards and the `[ suppressed ]` bracket on `/scan` and the homepage. `suppression-estimate.ts` and `aov-estimate.ts` are deleted outright (Amendment 1, same day: the ratified §2 kept the signal counting on the belief that the pillars consumed it — implementation disproved that; the pillars count independently and the modules had no other consumer). `suppressionEstimate` + `scaledSuppressionEstimate` leave the public `/api/scan` response (breaking change, no known consumer). FounderStrip keeps three panels; the recovery figure changes basis. Corrects ADR 0029's "`llms.txt` as a scored pillar" — it is a check inside Crawlability worth 40 of its points, not a pillar. Deliverable parity checked clean. |

> **Index gap, noted 2026-08-25:** rows for 0023, 0024, 0026 and 0027 are missing from this table though the ADRs exist on `origin/main`. Not introduced by 0028 or 0029; flagged rather than widened.
