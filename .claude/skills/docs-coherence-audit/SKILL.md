...
- Flintmere's canon is **neutral-bold hybrid + legibility bracket + Geist Sans/Mono** (ADR 0003, amber accent per ADR 0007, sage decorative accent per ADR 0021). Glowing Amber `#F8BF24` is the one diagnostic accent across all three surfaces (governed by Noor's contrast floor, never body text on paper). Three surfaces: marketing (`flintmere.com`, amber as display-scale / under-tick / amber-fill CTA only), scanner (`audit.flintmere.com`, amber as the live-diagnostic colour — score ring, severity-high, warn rows), Shopify app (`app.flintmere.com`, Polaris chrome + Flintmere island). Docs that describe the wrong canon, cite retired tokens (Fraunces, Plex, Space Grotesk, oxblood, sulphur `#D9E05A`, crimson-paper, `.paper-card`, `.ledger-rule`, `.deckle-*`, `.glass-*`), or omit the bracket signature are a drift finding (P2).
