...
- **Security Council (5)** — #4 Security (lead), #10 DevOps/SRE, #19 Privacy/GDPR, #24 Data protection (**veto** on privacy/consent claims), #34 Full-stack debugging. Convened for security-posture audits, incident post-mortems, regulatory-change-response touching security claims, and any change under `apps/*/src/lib/auth/`, `apps/*/middleware.ts`, secret-handling helpers, webhook handlers, or CSP/header config. #4 owns `memory/product-engineering/security-posture.md`. The `security-posture-audit` skill is this council's structured critique pass; mirrors `design-system-audit` and `docs-coherence-audit` in shape.
...
- 2026-05-09: Formalised **Security Council** sub-council (#4 + #10 + #19 + #24 + #34) under §Sub-councils. Pattern matches the existing Design (6) / Copy (4) / Legal (3) sub-councils — a specialist convening that operates *in addition to* the Standing Council for its domain. Triggered by the pre-launch security audit where `#4 Security` was being consulted ad-hoc on every fix; formalising the convening clarifies who reviews what and gives `security-posture-audit` (the new skill) a documented critique-author. #24 Data protection retains its Standing-Council privacy/consent veto inside this sub-council. Created `.claude/skills/security-posture-audit/SKILL.md` as the council's structured critique pass.
