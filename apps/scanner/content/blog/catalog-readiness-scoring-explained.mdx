...
faq:
  ...
  - q: "Why do missing GTINs matter so much?"
    a: "Google matches your products to its catalog using the GTIN. When a product that has an assigned GTIN omits it, Google cannot confirm what the product is, and the listing is more likely to be suppressed. That is why Identifiers shares the heaviest weight in the score, tied at the top with Attributes."
...
Picture a Shopify store with 1,800 products live, every one with photos, a price, and a description. Its Google Merchant Center account could still show only two-thirds of them as active, the rest sitting disapproved or limited — not shown to anyone searching Google Shopping. Nothing is broken on the website. The products simply fail the rules that decide whether a channel will show them, and no page in the Shopify admin tells you which ones or why. This is not a rare case; it is the default state of a catalog that has never been measured against the channels it feeds.
...
- **Identifiers** carries the most weight, because a missing GTIN is a common reason a product is suppressed.
...
A free Flintmere scan reports a partial score against these four. It is partial by design and labelled as such — it is a public scan, not the full Catalog Letter. The point is that you can see the shape of the problem in 60 seconds, for free, before deciding whether the rest is worth measuring.
...
That is the honest claim for a score. It is a measurement, sourced from the channels' own rules and from reading the catalog first-hand, that tells you where your data is leaking reach. The free scan reads the public 55 points in a minute. The rest is there when you want the full picture.
